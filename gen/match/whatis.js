...
const mgnlq_er_5 = require("mgnlq_er");
...
function makeSimplifiedSentences(aSentences, track) {
    return aSentences.sentences.map(function (oSentence) {
        ...
        var rWords = oSentence.filter(function (oWord) {
            ...
            return !mgnlq_er_5.Word.Word.isFiller(oWord);
        });
        track.fl += oSentence.length;
        track.lf += rWords.length;
        return {
    ...
    relevantRecords.forEach(function (record) {
        aSimplifiedSentences.forEach(function (aSentence) {
            ...
            aSentence.rWords.forEach(function (oWord) {
                ...
                else if (mgnlq_er_5.Word.Word.isCategory(oWord) && record[oWord.matchedString]) {
                    hasCategory[oWord.matchedString] = 1;
                }
                else if (!mgnlq_er_5.Word.Word.isCategory(oWord)) {
...
function processString(query, rules) {
    ...
    return mgnlq_er_3.ErBase.processString(query, rules, rules.wordCache);
    //  }
    /*
...
function analyzeContextString(contextQueryString, rules) {
    ...
    if (debuglog.enabled) {
        debuglog("after reinforce and cutoff" + aSentencesReinforced.sentences.length + "\n" + aSentencesReinforced.sentences.map(function (oSentence) {
            return mgnlq_er_4.Sentence.rankingProduct(oSentence) + ":" + JSON.stringify(oSentence);
        }).join("\n"));
    }
...
function cmpByNrCategoriesAndSameDomain(a, b) {
    ...
    var cnta = mgnlq_er_4.Sentence.getDistinctCategoriesInSentence(a).length;
    var cntb = mgnlq_er_4.Sentence.getDistinctCategoriesInSentence(b).length;
    /*
...
function analyzeCategoryMult(categorylist, rules, wholesentence, gWords) {
    ...
    res2.sort(mgnlq_er_4.Sentence.cmpRankingProduct);
    debuglog("resulting category sentences: \n", debuglog.enabled ? (mgnlq_er_4.Sentence.dumpNiceArr(res2.slice(0, 3), mgnlq_er_4.Sentence.rankingProduct)) : '-');
    ...
    var rescat = mgnlq_er_4.Sentence.getDistinctCategoriesInSentence(res2[0]);
    return rescat;
    // "" return res[0].filter()
...
function resolveCategories(categories, contextQueryString, theModel, domainCategoryFilter) {
    ...
    if (contextQueryString.length === 0) {
        return global.Promise.resolve({
            errors: [mgnlq_er_2.ErError.makeError_EMPTY_INPUT()],
            tupelanswers: [],
            tokens: []
        });
    }
        ...
        return ListAll.listAllTupelWithContext(categories, contextQueryString, theModel).then((res) => {
            //* sort by sentence
            res.tupelanswers.forEach(o => { o._ranking = o._ranking * mgnlq_er_4.Sentence.rankingProduct(o.sentence); });
